...
def process_file(filepath, header_import_path):
    with open(filepath, 'r') as f:
        content = f.read()
    
    # Check if Header is already imported
    if "import { Header }" in content:
        return
        
    # Find the header block
    header_pattern = re.compile(r'<header className="fixed top-0 z-50[^>]+>.*?</header>', re.DOTALL)
    ...
    import_stmt = f"import {{ Header }} from '{header_import_path}';\n"
...
def main():
    ...
    print("Created src/components/Header.tsx")
    ...
    for root, _, files in os.walk(routes_dir):
        for file in files:
            if file.endswith('.tsx'):
                ...
                rel_path = os.path.relpath('src/components/Header', root)
